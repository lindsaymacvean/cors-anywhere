...
function withCORS(headers, request) {
  //headers['access-control-allow-origin'] = process.env.CORSANYWHERE_WHITELIST.split(',')[0];
  headers['access-control-allow-origin'] = process.env.CORSANYWHERE_CLIENT;
  ...
  // If cookie returned from server, rewrite domain and path appropriately
  // https://github.com/Rob--W/cors-anywhere/issues/56#issuecomment-267905416
  if (headers['set-cookie']) {
    var setcookie = headers['set-cookie'][0].split(';');
    
    // Append ; because removed by split
    // setcookie[0] = setcookie[0]+''

    setcookie[1] = 'max-age=86400';

    // Proxy URL i.e. http://localhost:3000
    //setcookie[2] = 'domain='+process.env.CORSANYWHERE_CLIENT;

    // Final URL
    setcookie[2] = 'path=/'+process.env.CORSANYWHERE_SERVER;
    
    var cookieArray = [setcookie.join("; ")];
  ...
  //headers variable is set as proxyRes.headers from the outer function onProxyResponse
  var output = '';
  for (var property in headers) {
    output += property + ': ' + headers[property]+'; ';
  }
  console.log('These are the PROXY response headers coming back for '+i+' - '+request.url+' ... '+output+'\n');
  return headers;
}
...
function proxyRequest(req, res, proxy) {
  ...
  //   output += property + ': ' + req.headers[property]+'; ';
  // }
  // console.log('These are the CLIENT headers being sent for '+i+' - '+req.url+' ... '+output+'\n');
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  //   output += property + ': ' + res.headers[property]+'; ';
  // }
  // console.log('These are the FINAL response headers coming back for '+req.url+' ... '+output+'\n');
  return true;
}
