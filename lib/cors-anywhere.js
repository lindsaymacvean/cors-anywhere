...
function withCORS(headers, request) {
  ...
  //console.log('These are the request headers for '+request.url+' ... '+output);
  headers['access-control-allow-origin'] = process.env.CORSANYWHERE_WHITELIST.split(',')[0];
  // https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Allow-Origin
  headers['Vary'] = "Origin";
  var re = /.*\/sessions\/login.*/;
  if (re.test(request.url)) {
    headers['Access-Control-Allow-Credentials'] = true;
    //console.log('Added Access-Control-Allow-Credentials to response for request '+request.url);
  }
  ...
  // // Add cookie as proper header if returned from server
  if (headers['set-cookie']) {
    headers['Set-Cookie'] = headers['set-cookie'];
    console.log('THIS IS THE RESPONSE COOKIE FOR '+request.url+' : '+headers['set-cookie']);
  }
  ...
  console.log('These are the response headers coming back for '+request.url+' ... '+output);
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  console.log('A cookie is set'+ServerResponse.headers['Set-Cookie']);
  return true;
}
