...
function withCORS(headers, request) {
  ...
  if(re.test(request.url)) {
    console.log('Added Access-Control-Allow-Credentials to response '+request.url);
...
function relayRequestHeaders(proxyReq, req) {
  Object.keys(req.headers).forEach(function (key) {
    ...
    console.log('Request Header: '+req.headers[key]);
  });
}
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  console.log(proxyRes.url+' response had a cookie2: '+proxyRes.headers['set-cookie2']);
